...
| # | Cloud | Tool Stack | Managed/OSS | Key Tools |
|---|-------|-----------|-------------|-----------|
| 2.1 | AWS | Fully Managed | Buy | S3 + AWS Glue + Lake Formation + Athena + AWS Glue Catalog |
| 2.2 | AWS | OSS on Cloud | Build | S3 + Spark on EMR + Hive Metastore + Airflow + Presto/Trino |
| 2.3 | Azure | Fully Managed | Buy | ADLS Gen2 + Azure Data Factory + Purview + Synapse Serverless |
| 2.4 | Azure | OSS on Cloud | Build | ADLS + Spark on HDInsight/AKS + Apache Atlas + Trino |
| 2.5 | GCP | Fully Managed | Buy | GCS + Dataflow + Dataplex + BigQuery (external tables) |
| 2.6 | GCP | OSS on Cloud | Build | GCS + Spark on Dataproc + Hive Metastore + Trino |
| 2.7 | Multi-Cloud | OSS Portable | Build | S3/ADLS/GCS + Trino + Apache Atlas + Airflow |
| 2.8 | Hybrid | OSS Self-Hosted | Full Build | HDFS / MinIO + Hive + Spark + Ranger + Airflow |
...
| # | Cloud | Tool Stack | Managed/OSS | Key Tools |
|---|-------|-----------|-------------|-----------|
| 3.1 | AWS | Fully Managed | Buy | S3 + AWS Glue + Apache Iceberg + Redshift Spectrum + dbt Cloud |
| 3.2 | AWS | OSS on Cloud | Build | S3 + Iceberg + Spark on EMR + dbt Core + Airflow + Trino |
| 3.3 | Azure | Fully Managed | Buy | ADLS + Azure Databricks + Delta Lake + dbt Cloud + Synapse |
| 3.4 | Azure | OSS on Cloud | Build | ADLS + Delta Lake + Spark/Databricks OSS + dbt Core + Airflow |
| 3.5 | GCP | Fully Managed | Buy | GCS + BigLake + Iceberg + dbt Cloud + BigQuery |
| 3.6 | GCP | OSS on Cloud | Build | GCS + Iceberg + Spark on Dataproc + dbt Core + Airflow |
| 3.7 | Multi-Cloud | Fully Managed | Buy | Databricks (multi-cloud) + Delta Lake + dbt Cloud + Tableau |
| 3.8 | Multi-Cloud | OSS Portable | Build | S3/ADLS/GCS + Apache Iceberg + Spark + dbt Core + Airflow + Trino |
| 3.9 | Hybrid | OSS Self-Hosted | Full Build | MinIO + Apache Iceberg + Spark on K8s + dbt Core + Airflow |
...
1. Patterns 1, 3 — EDW and Lakehouse (highest enterprise demand, clearest variants)
2. Pattern 10 — Self-Serve Analytics Engineering (bridges 1 and 3)
3. Pattern 4 — Streaming Platform (second most common enterprise investment)
4. Pattern 5 — Data Mesh (organizational pattern, builds on 1/3)
5. Pattern 8 — AI/ML Platform (fastest growing)
6. Patterns 2, 6, 7 — Data Lake, Data Fabric, Operational Analytics
7. Patterns 9 — Governed/Compliance
